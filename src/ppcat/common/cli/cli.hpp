...
struct cli<std::tuple<T...>> {
    ...
    const config_t &get_config() const {
        return config;
    }
...
private:
    ...
    void define() {
        ...
        log::set_level();
        app.add_option_function<std::string>("-l,--log-level", [](const std::string &option) {
            if (not option.empty()) {
                log::set_level(option);
            }
        }, "Log level for logging, equal or severer wil be printed");

        app.add_option_function<std::string>("-f,--log-file" , [](const std::string &option) {
            if (not option.empty()) {
